    ...
    def lend_book(self, book, author):
        if book in self.list_of_books:
            if self.lend_data[book] is None:
                ...
                print("book lend on:",startdate, "return book by:",enddate)
            else:
    ...
    def return_book(self, book, author):
        if book in self.list_of_books:
            if self.lend_data[book] is not None:
                ...
                print("BOOK returned date:",enddate
                    )
            else:
...
def main():
    ...
    list_books = ['Maths', 'physics', 'chemistry', 'python']
    Library_name = 'vdit'
    secret_key = 123456

    vdit = Library(list_books, Library_name)

    print(
        f"Welecome To {vdit.library_name} library\n\nq for exit \nDisplay Book Using 'd' and add lend book using 'l' and Return a Book using 'r' \nAdd Book Using 'a' and Delete Book using 'del' \n ")
    ...
    while (Exit is not True):
        ...
        if _input == "q":
            ...
            vdit.lend_book(_input3, _input2)

        elif _input == "a":
            _input2 = input("Book name:")
            vdit.add_book(_input2)
            ...
            vdit.return_book(_input3, _input2)
